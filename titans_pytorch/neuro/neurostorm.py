...
try:
    ...
    Mamba = None
    Mamba2 = None
    print("Warning: mamba_ssm not found. NeuroSTORM will run in degraded mode (without Mamba).")
...
class SwinTransformerBlock4D(nn.Module):
    ...
    def __init__(
        ...
        if Mamba is not None:
            ...
            # Fallback when Mamba is missing: Use a simple Linear layer or Identity
            # Mamba preserves dimension: (B, L, D) -> (B, L, D)
...
class NeuroSTORMMAE(nn.Module):
    ...
    def random_masking(self, sequence):
        if self.spatial_mask == 'random' and self.time_mask == 'random':
            ...
            if num_mask > 0: sequence[overall_mask] = self.mask_token
            overall_mask = rearrange(overall_mask, '(B N) -> B N', B=B)
            overall_mask = overall_mask.cuda()
            new_sequence = rearrange(sequence, '(B D H W T) C -> B C D H W T', B=B, C=C, D=D, H=H, W=W, T=T)
            ...
            overall_mask = rearrange(overall_mask, 'B N -> (B N)')
            if num_mask > 0: windows[overall_mask] = self.mask_token
            overall_mask = rearrange(overall_mask, '(B N) -> B N', B=B)
            overall_mask = overall_mask.cuda()
